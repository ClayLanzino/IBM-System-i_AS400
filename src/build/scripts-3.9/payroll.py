...
def button_confirm():
    ...
    try:
        ...
        while (x < task):
            ...
            x+=1
            percent.set(str(int((x/task)*100))+"% Iniciando Interfaz. Por Favor, Espere Hasta Que El Proceso Termine.")
            WindowFrame.update_idletasks()

        completed_process = subprocess.run('python payroll_b.py')
        print(completed_process)

        # os.system('python payroll_b.py')
        # Testing environmnet ---> os.system('python payroll_in_progress.py')
    except:
...
percent_label = Label(WindowFrame, textvariable=percent).pack()
